  ...
  persist(
    (set) => ({
      ...
      completeOnboarding: (finalProfile) =>
        set((state) => ({
          onboardingComplete: true,
          partialProfile: { ...state.partialProfile, ...finalProfile, onboarding_complete: true },
        })),
...
export function readAuthState() {
  if (typeof window === 'undefined') return { userMode: null, onboardingComplete: false }
  try {
  ...
  return { userMode: null, onboardingComplete: false }
}
