...
const DIMENSION_META: Record<string, { label: string; color: string; description: string }> = {
  reliability:    { label: 'Reliability',    color: '#34d399', description: 'How consistently you deliver on commitments' },
  performance:    { label: 'Performance',    color: '#60a5fa', description: 'Quality and impact of your work output' },
  responsiveness: { label: 'Responsiveness', color: '#fbbf24', description: 'Speed and quality of communication' },
  feedback:       { label: 'Feedback',       color: '#f472b6', description: 'How clients and collaborators rate your work' },
  growth:         { label: 'Growth',         color: '#a78bfa', description: 'Your trajectory of learning and improvement' },
}
  ...
  if (!rec || !rec.content) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center gap-4">
        ...
        <Link href="/learn" className="text-sm text-violet-400 hover:text-violet-300 transition-colors">
    ...
    >
      ...
      <div className="rounded-2xl border border-border bg-card p-8">
        <div className="flex items-start gap-6 flex-wrap">
          {/* Icon */}
          <div className="w-16 h-16 rounded-2xl bg-violet-500/10 border border-violet-500/20 flex items-center justify-center flex-none">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="rounded-xl border border-border bg-card p-6">
              ...
              <div className="flex gap-4">
                <div className="w-10 h-10 rounded-xl bg-violet-500/10 border border-violet-500/20 flex items-center justify-center flex-none">
                  <reasonMeta.icon className="w-4 h-4 text-violet-400" />
                </div>
                <div>
                  ...
                  {rec.reason_text && (
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs text-violet-300/70">{rec.reason_text}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="text-xs font-bold text-violet-300">{rec.fit_score}%</span>
                  </div>
            ...
            <div className="rounded-xl border border-border bg-card p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {content.skill_tags.map(skill => (
                  <div
                    key={skill}
                    className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium bg-violet-500/10 border border-violet-500/20 text-violet-300"
                  >
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-violet-500/20 bg-violet-500/5 p-5 space-y-3">
            ...
            <button
              className="w-full flex items-center justify-center gap-2 rounded-lg bg-violet-500/20 border border-violet-500/30 px-4 py-2.5 text-sm font-medium text-violet-300 hover:bg-violet-500/30 transition-all"
            ...
            {content.url && (
              <a
                ...
                className="w-full flex items-center justify-center gap-2 rounded-lg border border-border px-4 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors"
              >
