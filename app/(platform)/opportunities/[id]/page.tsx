    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative w-36 h-36">
        ...
        <svg className="w-36 h-36 -rotate-90" viewBox="0 0 128 128">
          <circle cx="64" cy="64" r={r} fill="none" stroke="hsl(224,12%,18%)" strokeWidth="8" />
          <motion.circle
            cx="64" cy="64" r={r}
            fill="none"
            stroke={color}
  ...
  if (!opp) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center gap-4">
        ...
        <Link href="/opportunities" className="text-sm text-violet-400 hover:text-violet-300 transition-colors">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {opp.preferred_skills.map(skill => (
                    <span
                      key={skill}
                      className="rounded-lg px-2.5 py-1.5 text-xs font-medium bg-muted/40 text-muted-foreground border border-border/50"
                    >
                      {skill}
                    </span>
                  ))}
            ...
            <div className="rounded-xl border border-border bg-card p-6 space-y-4">
              ...
              {[
                ...
                <div key={label} className="flex gap-3">
                  <div className="w-7 h-7 rounded-lg bg-violet-500/10 border border-violet-500/20 flex items-center justify-center flex-none mt-0.5">
        ...
        {fit && (
          <div className="space-y-6">
            <div className="rounded-xl border border-border bg-card p-6 space-y-5">
              ...
              <ScoreBar label="Skill match" value={fit.skill_match ?? 0} color="#a78bfa" />
              <ScoreBar label="Signal match" value={fit.signal_match ?? 0} color="#60a5fa" />
              <ScoreBar label="Trajectory" value={fit.trajectory_match ?? 0} color="#34d399" />
              <ScoreBar label="Rate match" value={fit.rate_match ?? 0} color="#fbbf24" />
            </div>
            ...
            <div className="rounded-xl border border-violet-500/20 bg-violet-500/5 p-5 text-center">
              ...
              <button
                className="w-full rounded-lg bg-violet-500/20 border border-violet-500/30 px-4 py-2.5 text-sm font-medium text-violet-300 hover:bg-violet-500/30 transition-all"
