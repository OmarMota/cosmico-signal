...
export function ProfileSectionNav({ active, onSelect }: ProfileSectionNavProps) {
  ...
  return (
    <div ref={navRef} className="flex flex-col gap-1">
      {SECTIONS.map(({ key, label, icon: Icon, description }) => {
        ...
        return (
          <button
            ...
            className={cn(
              'flex items-center gap-3 rounded-xl px-3 py-2.5 text-left w-full transition-all duration-150',
              'border-l-2',
              isActive
                ? 'border-signal bg-signal/10 pl-[10px]'
                : 'border-transparent hover:bg-accent/60 pl-[10px]'
            )}
          >
            <div className={cn(
              'w-7 h-7 rounded-lg flex items-center justify-center flex-none transition-colors',
              isActive ? 'bg-signal/20' : 'bg-accent'
            )}>
              <Icon className={cn('w-3.5 h-3.5', isActive ? 'text-signal' : 'text-muted-foreground')} />
            </div>
            <div>
              <p className={cn('text-sm font-medium leading-none mb-0.5', isActive ? 'text-signal-light' : 'text-foreground')}>
                {label}
              </p>
      ...
      <div
        data-sidebar-item
        className="flex items-center gap-3 rounded-xl px-3 py-2.5 border-l-2 border-transparent pl-[10px] opacity-40 cursor-not-allowed"
        title="Available in a future release"
      >
        <div className="w-7 h-7 rounded-lg bg-accent flex items-center justify-center flex-none relative">
